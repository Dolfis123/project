...
function DetailSuratDomisili() {
  ...
  const [tempat_tanggal_lahir, setTtl] = useState(null);
  ...
  useEffect(() => {
    if (data) {
      setName(data.nama);
      setTtl(new Date(data.tempat_tanggal_lahir).toString());
  ...
  const handleFormSubmit = async (e) => {
    ...
    const formattedDate = format(tempat_tanggal_lahir, "yyyy-MM-dd");
    // Kirim permintaan ke backend untuk melakukan pengeditan
    try {
      const response = await axios.put(
        `http://localhost:3040/update-all-surat-tidak-mampu/${id}`,
        {
          nama: nama,
  ...
  const handleTerimaClick = async () => {
    try {
      await axios.post(`http://localhost:3040/terima-surat-skck/${id}`);
    ...
    if (selectSurat) {
      ...
      axios
        ...
        .then((res) => {
          ...
          window.location.href = "/permintaan-surat-tidak-mampu";
        })
        .catch((err) => console.log(err));
    }
  };
      ...
      <div className="back">
          ...
          <div className="card">
            <div className="card-header text-center">
              <h2 style={{ color: "#3468C0" }}>
                FORM EDIT BIODATA DIRI KETERANGAN TIDAK MAMPU
              </h2>
            </div>
            <br />
        ...
        <div className="container">
          <div className="card">
            <div className="card-header text-center">
              <h2 style={{ color: "#3468C0" }}>
                DETAIL BIODATA DIRI KETERANGAN TIDAK MAMPU
              </h2>
            </div>
            <br />
            <div className="card-body">
              ...
              <div style={{ display: "flex", marginBottom: "5px" }}>
                ...
                <span>
                  : {skckData.tempat_lahir},{" "}
                  {formatDate(skckData.tempat_tanggal_lahir)}
                </span>
              </div>
              <div style={{ display: "flex", marginBottom: "5px" }}>
                <span style={{ flex: "0 0 150px" }}>Jenis Kelamin </span>
                <span>: {skckData.jenis_kelamin}</span>
              </div>
              <div style={{ display: "flex", marginBottom: "5px" }}>
              ...
              >
                <Link to={`/detail-surat-tidak-mampu/${skckData.id}`}>
                  <button
                    style={{
                      display: "flex",
                      alignItems: "center",
