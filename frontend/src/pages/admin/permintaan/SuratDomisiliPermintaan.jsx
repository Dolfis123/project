...
function SuratDomisiliPermintaan() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3040/lihat-surat-menunggu-domisili"
      );
  ...
  const filteredData = data.filter((skck) =>
              ...
              {filteredData
                ...
                .map((skck, item) => (
                  <tr key={skck.id}>
                    ...
                    <td>{skck.nama}</td>
                    <td>{skck.jenis_surat}</td>
                    <td>{skck.rt_rw}</td>
                    <td>
                      <Link
                        to={`/detail-surat-domisili/${skck.id}`}
...
export default SuratDomisiliPermintaan;
