...
function ConfirmasiDataTidakMampu() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3040/lihat-surat-tidak-mampu/${hashed_id}`
      );
      ...
      <div className="container">
        <div className="card">
          <div className="card-header text-center">
            <h3 style={{ color: "#3468C0" }}>
              DETAIL BIODATA DIRI TIDAK MAMPU
            </h3>
          </div>
          <br />
          <br />
          <div className="card-body">
            ...
            <div style={{ display: "flex", marginBottom: "5px" }}>
              ...
              <span>: {formatDate(data.tempat_tanggal_lahir)}</span>
            </div>
            <div style={{ display: "flex", marginBottom: "5px" }}>
        ...
        <Link
          ...
          to={`/edit-tidak-mampu/${data.hashed_id}`}
        >
          <button className="btn btn-success text-white">Edit</button>
        </Link>
        <Link
          style={{ marginLeft: "100px" }}
          to={`/bukti-surat-tidak-mampu/${data.hashed_id}`}
        >
...
export default ConfirmasiDataTidakMampu;
