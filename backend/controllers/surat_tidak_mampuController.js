...
// Controller function untuk membuat surat domisili baru
const createDomisili = (req, res) => {
  ...
  upload.single("ktp_image")(req, res, (err) => {
    ...
    // Generate unique 5-character length surat number
    // const nomor_surat = generateUniqueNumber();
    // Generate nomor surat unik menggunakan UUID
    const nomor_surat = uuidv4();

    // SQL query to insert data into database
    const sqlQuery = `
    INSERT INTO surat_tidak_mampu
...
const getSuratTidakMampuMenunggu = (req, res) => {
  ...
  const sqlQuery = `
    SELECT * FROM surat_tidak_mampu where status_admin = 'menunggu'
  `;
...
const getSuratTidakMampuTerima = (req, res) => {
  // SQL query untuk mendapatkan semua data surat tidak mampu
  const sqlQuery = `
    SELECT * FROM surat_tidak_mampu where status_admin = 'diterima'
  `;
